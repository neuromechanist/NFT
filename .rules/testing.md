...
- **MATLAB (primary):** MATLAB `unittest` framework (`matlab.unittest`, class- or
  function-based tests under `tests/`). Drive the real pipeline end-to-end
  against the real fixtures in `../NFT_test` (real MRI, real 258-channel
  electrode file, legacy + FreeSurfer segmentations, BEM meshes). No synthetic
  heads, no fabricated leadfields.
- **Python (auxiliary only):** `pytest` with real fixtures, run via UV.
