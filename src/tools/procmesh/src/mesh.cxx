//	$Id: mesh.cxx,v 1.23 2008/04/14 16:11:39 canacar Exp $
#include <sys/types.h>
#include <limits.h>
#include <math.h>
#include "mesh.h"
...
TriMeshLin::loadMesh(const char *name)
{
	...
	if (correctMesh())
		MESH_LOG("Error correcting mesh!\n");
...
TriMeshLin::print_boundary(const char *hdr, const TriMeshLin::elist_t &blist)
{
	...
	/* Portability fix (SCCN, 07/2026): blist.size() is size_t, not int;
	 * %d on a 64-bit size_t is undefined behavior on LP64 platforms
	 * (silently truncates/misreads on the varargs stack). %zu is the
	 * correct C99/C++11 conversion for size_t and is behavior-preserving
	 * for every count this call ever prints. */
	printf("%s: boundary, %zu edges:\n", hdr, blist.size());
...
{
	const unsigned int *eu = getTri(elem);
	unsigned int v[3];
...
//---------------------------------------------------------------------------
void
TriMeshLin::pruneDeletedElements(void)
{
	unsigned int ne, s, d;
	if (!m_have_deleted_elem)
		return;

	MESH_LOG("Pruning Deleted Elements\n");


	ne = m_numtris;

	for (d = s = 0; s < m_numtris; s++) {
		if (isElemDeleted(s)) {
	...
	m_have_deleted_elem = 0;
	MESH_LOG("%d elements pruned\n", ne - m_numtris);
}
//---------------------------------------------------------------------------
void
...
{
	unsigned int e1, e2, n1, n2, p1, p2;
