...
//---------------------------------------------------------------------------
struct _pv {
	_pv(unsigned int n, const Point3 &v):_n(n), _v(v){};
	unsigned int _n;
	Point3 _v;
};
...
MeshProc::pushIntersecting(void)
{
	...
	for (EdgeIter it(*m_mesh); it.value(); it.next()) {
		...
		SCache::spset_t::iterator it2;
		for (it2 = eset.begin(); it2 != eset.end(); it2++) {
			unsigned int e = *it2;
			Point3 pi;
