...
class TriMeshLin {
 public:
	...
	void pruneDeletedElements(void);
	...
	inline const unsigned *getTriIndex(void) const
		{ return &(m_tris[0]); }
	inline const unsigned int *getTri(unsigned int tri) {
		return (m_tris[3 * tri] == (unsigned int) -1) ? NULL : &(m_tris[3 * tri]);
	}
	...
	inline void markElemDeleted(unsigned e){
		assert(e<m_numtris);
		m_tris[3*e]= (unsigned int) -1;
		m_have_deleted_elem = 1;
	}

	inline bool isElemDeleted(unsigned e){
		assert(e<m_numtris);
		return m_tris[3*e] == (unsigned int) -1;
	}
